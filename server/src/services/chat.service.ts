import type { ChatDemoResponse, ChatEchoResponse } from "../types/api";
...
export function createDemoReply(message: string): ChatDemoResponse {
  const normalizedMessage = message.trim();

  if (/学习路线|怎么学|学习计划/.test(normalizedMessage)) {
    return {
      ok: true,
      userMessage: normalizedMessage,
      assistantMessage:
  ...
  if (/项目|功能|范围/.test(normalizedMessage)) {
    return {
      ok: true,
      userMessage: normalizedMessage,
      assistantMessage:
  ...
  if (/技术选型|为什么|react|express|glm/i.test(normalizedMessage)) {
    return {
      ok: true,
      userMessage: normalizedMessage,
      assistantMessage:
