...
export function createDemoReply(input: DemoReplyInput): ChatDemoResponse {
  ...
  if (/学习路线|怎么学|学习计划/.test(latestUserMessage)) {
    return {
      ...
      assistantMessage: `${promptPrefix}建议先按“后端接口、前端联调、模型接入、工具调用、知识库检索”这个顺序推进。先把每一层的最小闭环跑通，再逐步替换成真实能力。`,
  ...
  if (/项目|功能|范围/.test(latestUserMessage)) {
    return {
      ok: true,
      userMessage: latestUserMessage,
      assistantMessage: `${promptPrefix}当前项目主线是 AI 工单 Copilot。首版重点是聊天、工具调用、知识库问答和基础评测，不做微服务和复杂多智能体。`,
  ...
  if (/技术选型|为什么|react|express|glm/i.test(latestUserMessage)) {
    return {
      ok: true,
      userMessage: latestUserMessage,
      assistantMessage: `${promptPrefix}当前技术选型优先考虑学习效率和完整闭环：前端用 React 19，后端用 Express 5，模型先用 GLM，后续再逐步增强。`,
      intent: "tech_choice",
  ...
  return {
    ok: true,
    userMessage: latestUserMessage,
    assistantMessage: `${promptPrefix}这是一个演示版聊天回复。当前还没有接入真实大模型，但接口结构已经接近后续真实聊天接口。`,
    intent: "general",
